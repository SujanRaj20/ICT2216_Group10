...
class Listing_Modules:  # Class to handle all listing-related functions
    ...
    @staticmethod
    def fetch_seller_orders(seller_id):
        engine = get_engine()
        try:
            query = """
            ...
            with engine.connect() as connection:
                result = connection.execute(text(query), {'seller_id': seller_id}).fetchall()
                orders = [dict(row) for row in result]
            return orders
        except SQLAlchemyError as e:
            current_app.logger.error(f"Error fetching orders for seller {seller_id}: {e}")
            return []
        finally:
            engine.dispose()
