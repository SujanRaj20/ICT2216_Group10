...
def get_log_files(log_dir='logs'):                                  # Function to get a list of log files
    ...
    return [f for f in os.listdir(log_dir) if f.startswith('app.')] # Return a list of log files in the logs directory by filtering files that end start with 'app.' in the directory
...
def get_dates_with_logs(log_dir='logs'):                            # Function to get a list of dates for which log files are present
    ...
    current_app.logger.debug(f"log_files: {log_files}")         
    
    # Check for 'app.log' and replace with today's date
    if 'app.log' in log_files:                                              # If 'app.log' is in the dates
        today = datetime.now(timezone('Asia/Singapore')).strftime("%Y-%m-%d")   # Get today's date in the Singapore timezone
        log_files.remove('app.log')                                         # Remove 'app' from the dates
        log_files.append('app.log.' + today)                                            # Replace it with today's date
    
    dates = {f.split('.')[2] for f in log_files}                    # Get the date from the log file name by splitting the file name at the '.' and taking the first part
...
def get_logs_within_date_range(start_date, end_date, log_dir='logs'):   # Function to get log files within a specified date range
    ...
    if 'app.log' in log_files:                                              # If 'app.log' is in the log files
        ...
        log_files.append('app.log.' + today)                                    # Add today's log file      
    ...
    for log_file in log_files:                      # Iterate through the log files
        log_date = log_file.split('.')[2]           # Get the date from the log file name by splitting the file name at the '.' and taking the first part
    ...
    if 'app.log.' + today in selected_files:            # If today's log file is in the selected files
        selected_files.remove('app.log.' + today)       # Remove today's log file
